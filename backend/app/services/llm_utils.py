...
def get_llm(model_choice: str = "gemini-2.5-flash"):
    """
    Get LLM instance for the specified model choice.
    
    Args:
        model_choice: Model name (default: gemini-2.5-flash)
        
    Returns:
        Initialized LLM instance ready for use
        
    Raises:
        ValueError: If model not found or required API keys missing
    """
    if not model_choice:
    ...
    try:
        ...
        if not config:
            logger.error(f"Model '{model_choice}' not found. Using default gemini-2.5-flash")
            model_choice = "gemini-2.5-flash"
            config = resolve_model_config(model_choice)
        
        if not config:
            raise ValueError(f"Could not resolve model configuration for {model_choice}")
        
        llm_class = config.get("class")
        constructor_params = config.get("constructor_params", {})
        
        # Merge with common params (don't override model-specific ones)
        final_params = {**_common_llm_params, **constructor_params}
        
        logger.info(f"Instantiating LLM: {model_choice}")
        
        return llm_class(**final_params)
    
    except Exception as e:
        logger.error(f"Failed to instantiate LLM {model_choice}: {str(e)}")
