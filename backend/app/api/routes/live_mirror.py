...
async def start_live_mirror(
    ...
    try:
        # No user_id anymore
        session_id = await manager.create_session(url, javascript_enabled)
...
async def live_mirror_websocket(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Initial snapshot failed for {session_id}: {init_err}")
            try:
                await websocket.send_json({"type": "error", "message": "Failed to capture initial snapshot"})
            except:
                pass
        ...
        while session.is_active:
            try:
                ...
                # Your existing action/refresh/navigate handlers...
                # (keep them as-is, but wrap sends in try/except if you want extra safety)
                ...
                try:
                    ...
                    logger.error(f"Periodic snapshot failed for {session_id}: {periodic_err}")

            except WebSocketDisconnect:
                logger.info(f"[LiveMirror] Client disconnected for session {session_id}")
                break

            except Exception as loop_err:
                logger.error(f"[LiveMirror] Loop error for {session_id}: {loop_err}")
                try:
                    await websocket.send_json({"type": "error", "message": "Internal session error"})
                except:
                    pass
                break

    except Exception as setup_err:
        logger.error(f"[LiveMirror] WebSocket setup error for {session_id}: {setup_err}")
        try:
            await websocket.send_json({"type": "error", "message": "Connection setup failed"})
        except:
            pass
    finally:
        # Safe, idempotent close - only attempt if still connected
        if websocket.client_state == WebSocketState.CONNECTED or websocket.application_state == WebSocketState.CONNECTED:
            try:
                await websocket.close(code=1000, reason="Normal closure")
                logger.info(f"[LiveMirror] WebSocket gracefully closed for {session_id}")
            except Exception as close_err:
                logger.debug(f"[LiveMirror] Close already handled or failed: {close_err}")
        else:
            logger.debug(f"[LiveMirror] WebSocket already closed/disconnected for {session_id}")
...
async def get_snapshot_diff(
    ...
    return {
        ...
        "compare": {
            ...
            "url": compare.current_url,
            "title": compare.title,
            "screenshot": compare.screenshot_base64xmh57jrknzkhv6y3ls3ubitzfqnkrwxhopf5aygthi7d6rplyvk3noyd
        },
        "text_diff": text_diff,
        # Visual diff is recommended to be calculated client-side using pixelmatch.js
    }
...
launched_tor_processes: Dict[str, subprocess.Popen] = {}


@router.post("/monitor/interact")
async def interact_onion(
    payload: dict = Body(...),
):
    """
    Launch Tor Browser with .onion URL and track the process for auto-close.
    Expects: {"url": "...", "session_id": "..."}
    """
    ...
    if not session_id:
        raise HTTPException(400, "Missing 'session_id' (required for tracking)")

    if not url.startswith(("http://", "https://")) or ".onion" not in url.lower():
        raise HTTPException(400, "Invalid .onion URL")

    script_path = "/home/eminent/Downloads/cipherflare/backend/scripts/launch-tor-browser.sh"
    ...
    if not os.access(script_path, os.X_OK):
        raise HTTPException(500, f"Script not executable: {script_path}\nRun: chmod +x {script_path}")

    try:
        # Launch and track the process
        process = subprocess.Popen(
            [script_path, shlex.quote(url)],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            start_new_session=True  # Separate process group for easier kill
        )

        launched_tor_processes[session_id] = process

        return {
            "success": True,
            "message": "Tor Browser launched",
            "url": url,
            "session_id": session_id
        }
...
@router.delete("/monitor/live/{session_id}")
async def stop_live_mirror(session_id: str):
    # Your existing session cleanup (manager.close_session, etc.)
    # ...

    # NEW: Kill Tor Browser process if tracked
    process = launched_tor_processes.pop(session_id, None)
    if process and process.poll() is None:  # Still running
        try:
            process.terminate()  # SIGTERM first (graceful)
            process.wait(timeout=5)
            logger.info(f"Terminated Tor Browser for session {session_id}")
        except subprocess.TimeoutExpired:
            process.kill()  # Force kill if needed
            logger.warning(f"Force-killed Tor Browser for session {session_id}")
        except Exception as e:
            logger.warning(f"Failed to kill Tor process {session_id}: {e}")
