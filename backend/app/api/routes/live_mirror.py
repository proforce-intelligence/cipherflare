...
# Global live mirror manager (singleton)
_live_mirror_manager: Optional[LiveMirrorManager] = None
...
async def start_live_mirror(
    ...
    """
    Start live mirroring of a .onion site — public endpoint
    """
    manager = get_live_mirror_manager()
    ...
    try:
        ...
        logger.info(f"[LiveMirror] Started anonymous session {session_id} for {url}")
...
async def navigate_live_mirror(
    ...
    """
    Navigate to a new URL within the live mirror session — public
    """
    manager = get_live_mirror_manager()
    session = manager.get_session(session_id)
...
async def stop_live_mirror(
    ...
    """
    Stop and close a live mirror session — public
    """
    manager = get_live_mirror_manager()
    session = manager.get_session(session_id)
...
async def live_mirror_websocket(websocket: WebSocket, session_id: str):
    """
    WebSocket endpoint for live mirror updates — public
    """
    await websocket.accept()
        ...
        while session.is_active:
            try:
                ...
                if msg_type in ["click", "type", "press_key", "scroll", "submit"]:
                    success = await session.execute_action(message)
                    if success:
                        await asyncio.sleep(0.8)  # delay for page reaction
                        snapshot_data = await session.get_snapshot()
                        await websocket.send_json({"type": "snapshot", "data": snapshot_data})

                        # Check alert after action
                        analysis = snapshot_data.get("analysis", {})
                        if analysis.get("risk_score", 0) >= 70:
                            await websocket.send_json({
                                "type": "high_risk_alert",
...
async def list_session_snapshots(
    ...
    """Get list of all saved snapshots for this session — public"""
    manager = get_live_mirror_manager()
    session = manager.get_session(session_id)
...
async def get_specific_snapshot(
    ...
    """Retrieve a specific historical snapshot — public"""
    manager = get_live_mirror_manager()
    session = manager.get_session(session_id)
...
async def get_snapshot_diff(
    ...
    """Get text diff between two snapshots — public"""
    manager = get_live_mirror_manager()
    session = manager.get_session(session_id)
    ...
    return {
        ...
        "text_diff": text_diff,
        # Visual diff should be calculated client-side
    }